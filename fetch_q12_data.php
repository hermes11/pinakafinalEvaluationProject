...
$q12_choices = array("YES", "NO");
$total_q12_row = get_total_rows($connect);
$output = '';
if($total_q12_row > 0)
{
	foreach($q12_choices as $row)
	{
		$query = "SELECT * FROM tbl_q12 WHERE q12_choices = '".$row."'";
		$statement = $connect->prepare($query);
		$statement->execute();
		$total_row = $statement->rowCount();
		$percentage_vote = round(($total_row/$total_q12_row)*100);
		$progress_bar_class = '';
