...
$q10_choices = array("YES", "NO");
$total_q10_row = get_total_rows($connect);
$output = '';
if($total_q10_row > 0)
{
	foreach($q10_choices as $row)
	{
		$query = "SELECT * FROM tbl_q10 WHERE q10_choices = '".$row."'";
		$statement = $connect->prepare($query);
		$statement->execute();
		$total_row = $statement->rowCount();
		$percentage_vote = round(($total_row/$total_q10_row)*100);
		$progress_bar_class = '';
