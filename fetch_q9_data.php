...
$q9_choices = array("YES", "NO");
$total_q9_row = get_total_rows($connect);
$output = '';
if($total_q9_row > 0)
{
	foreach($q9_choices as $row)
	{
		$query = "SELECT * FROM tbl_q9 WHERE q9_choices = '".$row."'";
		$statement = $connect->prepare($query);
		$statement->execute();
		$total_row = $statement->rowCount();
		$percentage_vote = round(($total_row/$total_q9_row)*100);
		$progress_bar_class = '';
