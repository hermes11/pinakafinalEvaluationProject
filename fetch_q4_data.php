...
$q4_choices = array("Strongly Dissatisfied", "Dissatisfied", "Neutral", "Satisfied", "Strongly Satisfied");
$total_q4_row = get_total_rows($connect);
$output = '';
if($total_q4_row > 0)
{
	foreach($q4_choices as $row)
	{
		$query = "SELECT * FROM tbl_q4 WHERE q4_choices = '".$row."'";
		$statement = $connect->prepare($query);
		$statement->execute();
		$total_row = $statement->rowCount();
		$percentage_vote = round(($total_row/$total_q4_row)*100);
		$progress_bar_class = '';
