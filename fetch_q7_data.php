...
$q7_choices = array("YES", "NO");
$total_q7_row = get_total_rows($connect);
$output = '';
if($total_q7_row > 0)
{
	foreach($q7_choices as $row)
	{
		$query = "SELECT * FROM tbl_q7 WHERE q7_choices = '".$row."'";
		$statement = $connect->prepare($query);
		$statement->execute();
		$total_row = $statement->rowCount();
		$percentage_vote = round(($total_row/$total_q7_row)*100);
		$progress_bar_class = '';
